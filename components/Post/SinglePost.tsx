...
const SinglePost = ({ post, currentUserId }: { post: any, currentUserId: string }) => {
            ...
            <View className="flex-row items-center justify-between px-5 mb-3">
                <View className="flex-row items-center">
                    <View className="w-10 h-10 bg-zinc-800 rounded-full overflow-hidden">
                        <Image source={{ uri: post.users?.profile_photo_url }} className="w-full h-full" />
                    </View>
                    <Text className="text-white font-bold ml-3 text-lg">{post.users?.username}</Text>
                </View>
            </View>

            <Image source={{ uri: post.image_url }} className="w-full aspect-square bg-zinc-900" />
